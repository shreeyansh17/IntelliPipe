...
def create_spark_session() -> SparkSession:
    ...
    return (
        ...
        .config("spark.sql.streaming.checkpointLocation", spark_conf.checkpoint_location)
        ...
        # Logging
        .config("spark.ui.enabled", "false")  # Disable in prod (use Spark History Server)
        .getOrCreate()
    )
...
class SchemaDriftDetector:
    ...
    def __init__(self, redis_client: redis.Redis, tenant_id: str, table_name: str) -> None:
...
class BatchDQMetrics:
    ...
    VALID_PAYMENT_METHODS = {"credit_card", "debit_card", "paypal", "crypto", "wire_transfer"}
    ...
    def compute_and_publish(
        ...
        critical_fields = ["event_id", "tenant_id", "order_id", "total_amount", "event_timestamp"]
        null_counts = batch_df.select([
            F.sum(F.when(F.col(c).isNull(), 1).otherwise(0)).alias(c)
            for c in critical_fields if c in batch_df.columns
        ]).collect()[0].asDict()
        avg_nulls = sum(null_counts.values()) / (len(null_counts) * total) if null_counts else 0
        ...
        if "order_status" in batch_df.columns and "payment_method" in batch_df.columns:
            ...
            valid_payment_expr = F.col("payment_method").isin(self.VALID_PAYMENT_METHODS)
            valid_count = batch_df.filter(valid_status_expr & valid_payment_expr).count()
        ...
        if all(c in batch_df.columns for c in amount_cols):
            consistent_rows = batch_df.filter(
                F.abs(
                    F.col("total_amount") -
                    (F.col("subtotal") + F.col("tax_amount") + F.col("shipping_amount"))
                ) <= 0.01
            ).count()
        ...
        self._redis.setex(cache_key, settings.redis.cache_ttl_seconds, json.dumps(scores))
        ...
        if overall < 80.0:
            alert = {
                ...
                "dq_score": overall,
                "severity": "critical" if overall < 60 else "high" if overall < 70 else "medium",
                "scores": scores,
                "detected_at": datetime.now(timezone.utc).isoformat(),
            }
            self._redis.lpush(settings.redis.alert_queue_key, json.dumps(alert))
            logger.warning("DQ score alert published", overall=overall, table=table_name)
...
class OrderStreamingPipeline:
    ...
    def _read_kafka_stream(self) -> DataFrame:
        ...
        return (
            ...
            .option("failOnDataLoss", "false")  # Handle topic partition deletion gracefully
